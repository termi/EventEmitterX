 ...
 */
export function stringifyWithCircularHandle(item: any, mapValueOrAutoHandle: boolean | ((key: string, val: unknown) => unknown) = false): string {
    let seenIndex = 0;
...
export function arrayContentStringify(
    ...
    for (let i = 0 ; i < length ; i++) {
        ...
        switch (type) {
            ...
            case "object":
                if (item) {
                    if (excludeObjectFn?.(item as { [key: string]: unknown })) {
                        result[i] = item;
                    }
                    else {
